...
router.post("/admin/initial-values", requireAuth, requireAdmin, verifyCsrf, (req, res) => {
  ...
  const upsert = db.prepare(
    `INSERT INTO member_initial_values (user_id, initial_alle9, initial_kranz, initial_carryover, initial_monte_points, initial_medaillen_gold, initial_medaillen_silver, initial_monte_siege, initial_medaillen_siege)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(user_id) DO UPDATE SET
       ...
       initial_medaillen_gold = excluded.initial_medaillen_gold,
       initial_medaillen_silver = excluded.initial_medaillen_silver,
  ...
  const trx = db.transaction(() => {
    for (const m of members) {
      ...
      const medaillenGold = Math.max(0, Number.parseInt(req.body[`initial_medaillen_gold_${m.id}`], 10) || 0);
      const medaillenSilver = Math.max(0, Number.parseInt(req.body[`initial_medaillen_silver_${m.id}`], 10) || 0);
      ...
      upsert.run(m.id, alle9, kranz, carryover, montePoints, medaillenGold, medaillenSilver, monteSiege, medaillenSiege);
    }
  });
  trx();
